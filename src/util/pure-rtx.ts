...
const logError = (des: string, err?: any) => {
  console.error(`${des}, err msg: `, err);
};
...
class Peer extends EventEmitter {
  ...
  constructor(opts?: PeerProps) {
    ...
    peerConnection.oniceconnectionstatechange = function (e) {
      ...
      if (iceConnectionState === "failed") {
        logError("iceConnectionState is failed", e);
      }

      if (iceConnectionState === "closed") {
        console.log("341312dsaasa");
      }
    };
  ...
  async signal(data: {
    ...
    const peer = this;
    if (!peer._pc) {
      throw new Error("signal error : RTCPeerConnection is not found");
      return;
    }
  ...
  send(data: Object) {
    ...
    try {
      ...
      logError("peer is not ready to send", e);
    }
  }
  ...
  start() {
    ...
    if (!peer.ready) {
      ...
      // if (isMozRTCPeerConnection) {
      //   setTimeout(peer._pc.onnegotiationneeded.bind(peer._pc), 0);
      // }
    }
  }
}
